...
# 指定縮放比例（1-10）
npm run start -- --dat "完整的.dat檔案路徑" --scale 2
```
...
# 處理 151 地圖並指定縮放比例為 2
npm run start -- --dat "D:\cg\map\1\1\151.dat" --scale 2

# 處理其他地圖並指定縮放比例為 3
npm run start -- --dat "D:\cg\map\2\5\205.dat" --scale 3
```
...
3. **scale 參數錯誤**
   ...
   ```bash
   ❌ scale 參數必須是 1 到 10 之間的整數
   ```

   解決：使用 `--scale` 指定有效的縮放比例，例如：`--scale 2`
